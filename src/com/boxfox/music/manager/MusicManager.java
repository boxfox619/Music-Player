...
import com.boxfox.music.YoutubeMusic;
...
public class MusicManager extends BaseManager {
	...
	public void playMusic(YoutubeMusic yMusic) {
		update(new Event(MusicManager.class, "음악을 다운로드중입니다..."));
		new SwingWorker(){
			@Override
				...
				try {
					...
					javafx.application.Platform.runLater(() -> {
						try {
							...
							count++;
							for (int i = 0; i < 5; i++) {
								String str = (String) engine.executeScript("document.getElementById('dl_link').childNodes[" + i + "].href;");
